...
const Button: React.FC<ButtonProps> = (props) => {
  return (
    <button onClick={props.onClick} className="bg-stone-300 p-2 hover:bg-sky-700" type="submit">{props.children}</button>
  )
}
      ...
      <div className="m-auto flex flex-col items-center justify-between gap-8 h-full w-11/12">
        ...
        <ActionBar>
          {!showZineForm && !currentZine ? (
            <Button onClick={() => { setCurrentZine(null); setShowZineForm(true) }}>New Zine</Button>
          ) : <></>}
          {!!currentZine ? (
            <Button onClick={() => onDeleteZineSubmit(currentZine)}>Delete Zine</Button>
          ) : <></>}
          {(showZineForm || currentZine || showZineForm) ? (
            <Button onClick={() => { setCurrentZine(null); setShowZineForm(false) }}>Back</Button>
          ) : <></>}
        </ActionBar>
